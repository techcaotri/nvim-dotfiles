...
local toggleterm_config = {
  ...
  -- toggleterm_config.execs = {...} to overwrite
  -- toggleterm_config.execs[#toggleterm_config.execs+1] = {"gdb", "tg", "GNU Debugger"}
  -- TODO: pls add mappings in which key and refactor this
  execs = {
    { nil, "<M-h>", "Horizontal Terminal", "horizontal", 0.3 },
    { nil, "<M-v>", "Vertical Terminal",   "vertical",   0.4 },
    { nil, "<M-i>", "Float Terminal",      "float",      nil },
  },
}
...
local function get_dynamic_terminal_size(direction, size)
  size = size or toggleterm_config.size
...
M.init = function()
  for i, exec in pairs(toggleterm_config.execs) do
    local direction = exec[4] or toggleterm_config.direction

    local opts = {
      cmd = exec[1] or toggleterm_config.shell or vim.o.shell,
      keymap = exec[2],
      label = exec[3],
